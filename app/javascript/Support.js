...
Support.updateURLHistory = function(page,title,url,title2,url2,selectedItem,topLeftItem,isTop,playlistId,parentItem) {
	...
	if (this.previousPageDetails.length > 0) {
		...
		if (this.previousPageDetails[this.previousPageDetails.length-1][8] != playlistId) {
			this.previousPageDetails.push([page,title,url,title2,url2,selectedItem,topLeftItem,isTop,playlistId,parentItem]);
...
Support.processReturnURLHistory = function(bookmark) {
	...
	if (this.previousPageDetails.length > 0) {
		...
		var isTop = array[7];
		var playlistId = array[8];
		var parentItem = array[9];
		
		//Handle Navigation?
		switch (page) {
			case "GuiDisplay_MediaItems":
				GuiDisplay_MediaItems.start(title,url,selectedItem,topLeftItem,null,playlistId,parentItem);
				break;
			case "GuiPage_ItemDetails":
				title.bookmark = bookmark;
				GuiPage_ItemDetails.start(title,title2);
				break;	
			case "GuiPage_Search":
				GuiPage_Search.start(title,url,playlistId);
				break;
			default:
				break;
		}
...
Support.processIndexing = function(ItemsArray) {	
	...
	var checkForNonLetter = ItemsArray[0].title.charAt(0).toLowerCase();	
	...
	for (var index = 0; index < ItemsArray.length; index++) {	
		var letter = ItemsArray[index].title.charAt(0).toLowerCase();	
...
Support.updateDisplayedItems = function(Items,selectedItemID,startPos,endPos,DivIdUpdate,DivIdPrepend,isResume,Genre,showBackdrop) {
	...
	for (var index = startPos; index < endPos; index++) {
		if (isResume == true) {
			var hms = Items[index].duration;   // your input string
			...
			if (Items[index].type == "video" && Items[index].episode != 0) {
				var title = Items[index].title;		
				var imgsrc = "";
				if (Items[index].poster) {
					title = Items[index].title;
					imgsrc = Items[index].poster;
				} else if (Items[index].backdrop) {	
					title = Items[index].title;
					imgsrc = Items[index].backdrop;
				} else {
			...
			if (Items[index].type == "Audio" || Items[index].type.indexOf("audio") == 0){
				var title = Items[index].title;
				if (Items[index].poster) {	
					var imgsrc = Items[index].poster;
					var hms = Items[index].duration;   // your input string
			...
			//----------------------------------------------------------------------------------------------
				...
				if (Items[index].poster) {		
					...
					if (hms != null) {
						...
						var bookmarkseconds = Items[index].bookmark;   // your input string
					...
					htmlToAdd += "<div id='"+ DivIdPrepend + Items[index].id + "' style='background-image:rgba(0,0,0,0.5)'><div class=menuItem>"+ title + "</div></div>";				
				}
			
				//Add watched and favourite overlays.
				//if (Items[index].lastPlayed != "0") {
				//	htmlToAdd += "<div class='genreItemCount highlightMezzmoBackground'>&#10003</div>";	
				//} 
				//htmlToAdd += "</div>";
			//----------------------------------------------------------------------------------------------	
			} else if (Items[index].type == "container" || Items[index].type == "Folder") {
				var title = Items[index].title;
				if (Items[index].poster) {		
					var imgsrc = Items[index].poster;
			...
			//----------------------------------------------------------------------------------------------
				...
				if (Items[index].poster) {		
					...
					htmlToAdd += "<div id='"+ DivIdPrepend + Items[index].id + "' style='background-image:url(\"" +imgsrc+ "\");background-repeat:no-repeat;background-size:contain;'><div class=menuItem>"+ title + "<br><div class='item_t'>" + Items[index].ipAddress + "</div></div></div>";
				} else {
					htmlToAdd += "<div id='"+ DivIdPrepend + Items[index].id + "' style='background-color:rgba(0,0,0,0.5);'><div class=menuItem>"+ title + "</div></div>";
				}			
			} else if (Items[index].type == "Trailer") {
				var title = Items[index].title;	
				if (Items[index].poster != "") {	
					var imgsrc = Items[index].poster;
					/*var totalseconds = Items[index].duration;   // your input string
					if (totalseconds != 0) {
						var minutes = Support.convertTicksToMinutes(totalseconds* 1000);
						htmlToAdd += "<div id='"+ DivIdPrepend + Items[index].id + "'><div class='item_a'><div class='item_i'><img class='icon_i' src='" + imgsrc + "'></img></div></div><div class=menuItemBlockSmall>"+ title + "<br><div class='item_t'>" + minutes + "</div></div></div>";
					}
					else {*/
			...
			//----------------------------------------------------------------------------------------------
			} else if (Items[index].type == "actor") {
				var title = Items[index].title;	
				if (Items[index].poster != "") {	
					var imgsrc = Items[index].poster;
	...
	var elem = document.getElementById(DivIdUpdate);
	if (elem != null) {
		elem.innerHTML = htmlToAdd;
	}
}
...
Support.updateSelectedNEW = function(Array,selectedItemID,startPos,endPos,strIfSelected,strIfNot,DivIdPrepend,dontUpdateCounter,totalRecordCount) {
	for (var index = startPos; index < endPos; index++){		
		if (index == selectedItemID) {	
			var elem = document.getElementById(DivIdPrepend + Array[index].id);
			if (elem != null) {
				elem.style.zIndex = "5";
				elem.className = strIfSelected;			
			}
		} else {	
			var elem = document.getElementById(DivIdPrepend + Array[index].id);
			if (elem != null) {
				elem.style.zIndex = "2";
				elem.className = strIfNot;		
			}
		}			
    }
...
Support.processSelectedItem = function(page,ItemData,startParams,selectedItem,topLeftItem,isTop,genreType,isLatest) {	
	if (ItemData[selectedItem].MediaType != "Audio") {
		if (page == "GuiPage_HomeTwoItems") {
			...
			if (startParams == null) {
				...
				Support.updateURLHistory(page,startParams[0],startParams[1],null,null,selectedItem,topLeftItem,null,ItemData[selectedItem].parentId,startParams[2]);
			}
			
		}
	}
	
	alert("Type: "+ItemData[selectedItem].type +" MediaType: "+ItemData[selectedItem].MediaType);
	switch (ItemData[selectedItem].type) {	
	case "Movie":
		
		if (page == "GuiDisplay_MediaItems"){
			GuiPage_ItemDetails.start(ItemData[selectedItem]);
		} else {
			GuiPage_ItemDetails.start(ItemData[selectedItem]);
		}
		break;	
	...
	case "container":
		...
		GuiDisplay_MediaItems.start(ItemData[selectedItem].title,url, 0,0, null, ItemData[selectedItem].id,ItemData[selectedItem]);
		break;	
	default:
		switch (ItemData[selectedItem].MediaType) {
		case "Photo":
		...
		case "Audio":
			...
			var url = ItemData[selectedItem].contentUrl;
			GuiMusicPlayer.start("Songs",selectedItem,page,false,false,ItemData);
			break;
		default:
...
Support.playSelectedItem = function(page,ItemData,startParams,selectedItem,topLeftItem,isTop) {
	...
	alert ("playSelectedItem: Type "+ItemData[selectedItem].type);
	if (ItemData[selectedItem].type == "Folder") {
		if (page == "GuiPage_Photos") {
			Support.updateURLHistory(page,startParams[0],startParams[1],startParams[2],startParams[3],selectedItem,topLeftItem,isTop);
			GuiImagePlayer.start(ItemData,selectedItem,true);	
		}
	} else if (ItemData[selectedItem].MediaType == "Video" && ItemData[selectedItem].type != "TvChannel" && ItemData[selectedItem].type != "Playlist") {
		if (ItemData[selectedItem].LocationType == "Virtual"){
			return;
		}
		Support.updateURLHistory(page,startParams[0],startParams[1],startParams[2],startParams[3],selectedItem,topLeftItem,isTop);
		var url = ItemData[selectedItem].contentUrl;
		GuiPlayer.start("PLAY",url,ItemData[selectedItem].UserData.PlaybackPositionTicks / 10000,page);	
	} else if (ItemData[selectedItem].type == "Playlist") {
		var url = Server.getCustomURL("/Playlists/"+ItemData[selectedItem].id+"/Items?userId="+Server.getUserID()+"&StartIndex=0&SortBy=SortName&SortOrder=Ascending&fields=ParentId,SortName,MediaSources");
		if (ItemData[selectedItem].MediaType == "Video"){
			Support.updateURLHistory(page,startParams[0],startParams[1],startParams[2],startParams[3],selectedItem,topLeftItem,isTop);
			GuiPlayer.start("PlayAll",url,0,page);
		} else if (ItemData[selectedItem].MediaType == "Audio"){
			GuiMusicPlayer.start("Album",url,page,false);
		} else {
			return;
		}
	} else if (ItemData[selectedItem].MediaType == "ChannelVideoItem") {
		Support.updateURLHistory(page,startParams[0],startParams[1],startParams[2],startParams[3],selectedItem,topLeftItem,isTop);
		var url = ItemData[selectedItem].contentUrl;
		GuiPlayer.start("PLAY",url,ItemData[selectedItem].UserData.PlaybackPositionTicks / 10000,page);	
	}  else if (ItemData[selectedItem].type == "TvChannel") {
		Support.updateURLHistory(page,startParams[0],startParams[1],startParams[2],startParams[3],selectedItem,topLeftItem,isTop);
		var url = ItemData[selectedItem].contentUrl;
		GuiPlayer.start("PLAY",url,0,page);
	}  else if (ItemData[selectedItem].CollectionType == "photos") {
		Support.updateURLHistory(page,startParams[0],startParams[1],startParams[2],startParams[3],selectedItem,topLeftItem,isTop);
		GuiImagePlayer.start(ItemData,selectedItem,true);	
	} else if (ItemData[selectedItem].type == "PhotoAlbum") {
		Support.updateURLHistory(page,startParams[0],startParams[1],startParams[2],startParams[3],selectedItem,topLeftItem,isTop);
		GuiImagePlayer.start(ItemData,selectedItem,true);	
	} else if (ItemData[selectedItem].type == "Series") {
		Support.updateURLHistory(page,startParams[0],startParams[1],null,null,selectedItem,topLeftItem,null);
		var url= ItemData[selectedItem].contentUrl;
		GuiPlayer.start("PlayAll",url,0,page);
	} else if (ItemData[selectedItem].type == "Season") {
		Support.updateURLHistory(page,startParams[0],startParams[1],null,null,selectedItem,topLeftItem,null);
		var urlToPlay= ItemData[selectedItem].contentUrl;
		GuiPlayer.start("PlayAll",urlToPlay,0,page);	
	} else if (ItemData[selectedItem].type == "Movie" || ItemData[selectedItem].type == "Episode") {
		Support.updateURLHistory(page,startParams[0],startParams[1],null,null,selectedItem,topLeftItem,null);
		var url = ItemData[selectedItem].contentUrl;
		GuiPlayer.start("PLAY",url,0,page);
	} else if (ItemData[selectedItem].type == "MusicAlbum") {
		Support.updateURLHistory(page,startParams[0],startParams[1],null,null,selectedItem,topLeftItem,null);
		var url = Server.getChildItemsURL(ItemData[selectedItem].id,"&SortBy=SortName&SortOrder=Ascending&IncludeItemTypes=Audio&Recursive=true&CollapseBoxSetItems=false&Fields=MediaSources");
		GuiMusicPlayer.start("Album",url,"GuiDisplay_MediaItems",false);
	} else if (ItemData[selectedItem].type == "Audio") {
		Support.updateURLHistory(page,startParams[0],startParams[1],null,null,selectedItem,topLeftItem,null);
		var url = ItemData[selectedItem].contentUrl;
		GuiMusicPlayer.start("Song",url,"GuiDisplay_MediaItems",false);
	}
}
...
Support.getLibraryItem = function (itemData, NumberReturned, TotalMatches) {
	if (itemData.length != 0) {
		alert("LibraryItem: " + itemData[0].title + " Desc: " + itemData[0].Overview);
		GuiDisplay_MediaItems.start(itemData[0].title,Server.getServerAddr(), 0,0, null, "0",itemData[0]);
	}
	else {
		//hide Loading Div
		document.getElementById("loading").style.visibility = "hidden";
		...
		//GuiNotifications.setNotification("Error : Library not found!","Error",true);
...
Support.processHomePageMenu = function (menuItem) {
	//GuiPage_HomeOneItem.start("Mezzmo",Server.getServerAddr(),0,0);
	//return;
	switch (menuItem) {
	case "Home":
		...
		try {
			...
			var itemList = [];
			Server.Browse(Server.getServerAddr(), "Browse", "0", "BrowseMetadata", 0, 1, "", itemList, Support.getLibraryItem);
		}
		catch (err) {
			//hide Loading Div
			document.getElementById("loading").style.visibility = "hidden";
...
Support.fadeImage = function(imgsrc) {
	...
	if (bg != imgsrc) {
		...
		setTimeout(function(){
			...
			document.getElementById("pageBackground").style.opacity = "1.0";
		}, 350);
	}	
}
...
Support.convertTicksToMinutes = function (currentTime) {
	...
	if (timeMinute != 0) {
		...
		timeMinute = Math.floor((currentTime / 3600000) * 60 * 60);
		return timeMinute + " secs";
	}
}
...
Support.getCurrentDate = function() {
	var today = new Date();
...
Support.getSeconds = function(hms) {
	var a = hms.split(':'); // split it at the colons
